function [det_obj_act_boxes] = vic_compute_zeroshotBoxes(GT_List, cls_zeroshot, paths, options)
...
for ii=1:n_frames
    det = [];
    det = load([paths.test_detections GT_List{ii} '_ZeroShotObjectClass' num2str(cls_zeroshot) '.mat']);   
    ...
    for cls_obj = 1:options.c_obj       
        ...
        bboxes = bboxes + 1; % boxes from python to matlab format (starting at 0)
